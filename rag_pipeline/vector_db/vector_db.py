...
class MyWeaviateDB:
    ...
    def store(self, chunk: dict[str, str], embeddings: SentenceTransformer) -> None:
        """
        ...
        try:
            ...
            if chunk.get("contextual_content"):
                text_to_embed = f"passage: {chunk.get('contextual_content')}"
            else:
                text_to_embed = f"passage: {properties['content']}"
    ...
    def search(self, query: str, embeddings: SentenceTransformer, alpha: float = 1, limit: int = 5):
        """
